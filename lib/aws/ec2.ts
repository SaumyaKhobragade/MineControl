...
export async function getInstanceStatus(instanceId: string): Promise<{ state: EC2State; launchTime?: Date }> {
  ...
  try {
    ...
    throw new AWSOperationError(`Failed obtaining status for ${instanceId}: ${message}`, error);
  }
}
...
export async function startInstance(instanceId: string): Promise<void> {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : String(error);
    throw new AWSOperationError(`Failed executing start instance request for ${instanceId}: ${message}`, error);
  }
}
...
export async function stopInstance(instanceId: string): Promise<void> {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : String(error);
    throw new AWSOperationError(`Failed executing stop instance request for ${instanceId}: ${message}`, error);
  }
}
