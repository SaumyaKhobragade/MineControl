...
export async function runCommand(instanceId: string, command: string): Promise<string> {
  ...
  try {
    ...
    throw new AWSOperationError(`Failed executing SSM command on ${instanceId}: ${message}`, error);
  }
}
...
export async function runCommandWithOutput(
  ...
  try {
    ...
    while (Date.now() - startTime < timeoutMs) {
      ...
      try {
        ...
        if (!message.includes("InvocationDoesNotExist")) {
          console.warn(`[ssm] Error getting command invocation: ${message}`);
        }
      }
    }
    ...
    const message = error instanceof Error ? error.message : String(error);
    throw new AWSOperationError(`Failed executing SSM command with output on ${instanceId}: ${message}`, error);
  }
}
...
export async function stopMinecraft(
  ...
  for (let attempt = 1; attempt <= 12; attempt++) {
    ...
    try {
      ...
      console.warn(`[STOP] Verification attempt ${attempt}/12 failed to query process: ${message}`);
    }
  }
...
export async function waitForSSM(
  ...
  while (Date.now() - startTime < timeoutMs) {
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      console.warn(`[ssm] Error checking instance information status: ${message}`);
    }
